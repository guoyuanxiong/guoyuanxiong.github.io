...
VERSIONED_ASSETS = ('assets/site/styles.css', 'assets/site/main.js')


def version_assets(page):
    """Append ?v=<content hash> to the stylesheet and script so browsers
    fetch the new files as soon as they change, instead of pairing a fresh
    index.html with a cached copy of the old CSS or JavaScript."""
...
NEWS_VISIBLE = 5
NEWS_LIST = re.compile(r'(<ol class="news-list">)(.*?)(\s*</ol>)', re.S)
NEWS_ITEM = re.compile(r'\s*<li>.*?</li>', re.S)


def fold_earlier_news(page):
    """Show the five newest News entries; put older ones behind a
    "View earlier news" toggle (same <details> pattern as Service)."""
    def fold(match):
        start, body, end = match.groups()
        items = NEWS_ITEM.findall(body)
        if len(items) <= NEWS_VISIBLE or ''.join(items).strip() != body.strip():
            return match.group(0)
        shown, earlier = items[:NEWS_VISIBLE], items[NEWS_VISIBLE:]
        return (start + ''.join(shown) + end +
                '\n  <details class="news-more">\n   <summary>View earlier news</summary>\n'
                '   <ol class="news-list">' + ''.join(earlier) + '\n   </ol>\n  </details>')
    return NEWS_LIST.sub(fold, page, count=1)
...
def standalone(page):
    ...
    page, css_count = re.subn(r'<link\b[^>]*href="assets/site/styles\.css(?:\?v=\w+)?"[^>]*/?>', lambda _: '<style>\n' + css + '\n</style>', page)
    page, js_count = re.subn(r'<script\b[^>]*src="assets/site/main\.js(?:\?v=\w+)?"[^>]*>\s*</script>', lambda _: '<script>\n' + js + '\n</script>', page)
